...
<br>Inserting into a linked list takes 4 basic steps.

### For Inserting into the Front of the List
<b>Note:</b> be sure to check that the list isn't empty before doing these steps. (`self.head is None`). If so, just set the head and tail pointers to the new node
...
### For Inserting into the Back of the List
1. Create a new node (`new_node = LinkedList.Node(value)`)
...
Inserting in the middle gets a bit more complicated. <b>Note:</b> insertion comes after the chosen node. We'll call it `current` down below. 
1. Create a new Node (`new_node = LinkedList.Node(value)`)
...
Now for Removing. It gets a bit simpler in steps here, but each step is a bit... more complicated to undertand.
...
<b>Note:</b> We are using the name `current` here again.
...
class Chain:
    """
    The linked list is best used as a class, so it is created here. Linked lists consist of Nodes, so Node is a class that will exist inside the LinkedList class
    ...
    class Link:
        ...
        def __init__(self, data):
            """
            Since this is initialization, there is nothing for the next and prev pointers to point to.
            """
